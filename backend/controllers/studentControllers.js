...
const createStudent = async (req, res) => {
    try {
        ...
        if (!name || !email || !course || !gender || !age) {
            return res.status(400).json({ message: 'Name, email, course, gender, and age are required' });
        }

        const id = uuidv4();
        const now = new Date().toISOString();
        const student = { id, name, email: email.toLowerCase().trim(), course, gender: gender.trim(), age: parseInt(age), createdAt: now, updatedAt: now };
...
const updateStudent = async (req, res) => {
    try {
        ...
        if (age) updates.age = age.toString();
